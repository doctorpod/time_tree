...
module TimeTree
  ...
  describe DateCalculator do
    ...
    describe 'date_between' do
      ...
      it 'between' do
        date_between?(jun2, jun1, jun3).should be true
      end

      it 'outside' do
        date_between?(jun1, jun2, jun3).should be false
      end

      it 'at start' do
        date_between?(jun1, jun1, jun3).should be true
      end

      it 'at end' do
        date_between?(jun3, jun1, jun3).should be true
      end
    end
    ...
    describe 'in_prev_week?' do
      ...
      it 'within - zero count' do
        in_prev_week?(may7, 0, may7).should be true
      end

      it 'within - 1 count' do
        in_prev_week?(may1, 1, may7).should be true
      end

      it 'outside' do
        in_prev_week?(may7, 1, may7).should be false
      end
    end
    ...
    describe 'in_prev_month?' do
      ...
      it 'within - zero count' do
        in_prev_month?(may1, 0, may7).should be true
        in_prev_month?(may31, 0, may7).should be true
      end

      it 'within - 1 count' do
        in_prev_month?(apr1, 1, may7).should be true
      end

      it 'outside' do
        in_prev_month?(apr1, 0, may7).should be false
      end
    end
  end
